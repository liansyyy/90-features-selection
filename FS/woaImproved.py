...
def binary_conversion_reverse(X, thres, N, dim):
...
def jfs(xtrain, ytrain, opts):
    ...
    Xbin = binary_conversion_reverse(Xbin, thres, N, dim)
    ...
    while t < max_iter:
        ...
        for i in range(N):
            ...
            # Paramater C (2.4)
            C = 2 * rand()
            # Parameter p, random number in [0,FeatureSelection]
            p = rand()
            # Parameter l, random number in [-FeatureSelection,FeatureSelection]
            l = -1 + 2 * rand()
            # Whale position update (2.6)
            if p < 0.5:
                # {FeatureSelection} Encircling prey
                if abs(A) < 1:
                    for d in range(dim):
                        # Compute D (2.FeatureSelection)

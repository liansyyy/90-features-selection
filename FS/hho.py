...
def jfs(xtrain, ytrain, opts):
    ...
    while t < max_iter:
        ...
        for i in range(N):
            # Random number in [-FeatureSelection,FeatureSelection]
            E0 = -1 + 2 * rand()
            # Escaping energy of rabbit (3)
            E  = 2 * E0 * (1 - (t / max_iter)) 
            # Exploration phase
            if abs(E) >= 1:
                # Define q in [0,FeatureSelection]
                q = rand()
                if q >= 0.5:
                    ...
                    r2 = rand()
                    for d in range(dim):
                        # Position update (FeatureSelection)
